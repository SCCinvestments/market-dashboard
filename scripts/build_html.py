...
function loadKospiHeatmap() {
    const isLight = document.body.getAttribute('data-theme') === 'light';
    const theme = isLight ? 'light' : 'dark';
    
    const container = document.getElementById('kospiHeatmapContainer');
    container.innerHTML = '<div class="tradingview-widget-container"><div class="tradingview-widget-container__widget"></div></div>';
...
<div class="update-time"><span class="live-dot"></span>데일리 리포트 · {updated_at}</div>
</div>
